...
4. Apply and verify the trusted proxy/client-address configuration in `deploy/PRODUCTION_SECURITY.md`. The limiter intentionally does not parse X-Forwarded-For; Nginx must overwrite forwarding headers, Tomcat must trust only loopback Nginx, and port 8080 must remain private. Shared NATs still share anonymous budgets.
5. Apply and verify the H2/backup filesystem permissions in `deploy/PRODUCTION_SECURITY.md`. Default local H2 credentials are not a production filesystem-security boundary. No network H2 console/server is configured by this source; verify none is enabled externally.
